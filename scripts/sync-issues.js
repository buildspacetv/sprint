...
async function main() {
  ...
  for (let page = 1; page <= 10; page++) {
    const batch = await api(`https://api.github.com/repos/${REPO}/issues?labels=submission&state=open&per_page=100&page=${page}`);
  ...
  console.log(`${projects.length} submission(s) synced${next === prev ? ' (no change)' : ''}`);
}
